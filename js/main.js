...
// =================================================================
//                 NAV BAR ANIMATION EFFECTS (REVISED)
// =================================================================
document.addEventListener('DOMContentLoaded', () => {

  const animatedNavLinks = document.querySelectorAll('.fx-shine-sweep');

  function masterLoop() {
    // 同时为所有链接添加 'sweep' 类来触发动画
    animatedNavLinks.forEach(link => {
      const sheen = link.querySelector('.fx-sheen');
      link.classList.add('sweep');
      sheen.classList.add('sweep');
    });

    // 在动画结束后（900ms），移除 'sweep' 类，为下一次循环做准备
    setTimeout(() => {
      animatedNavLinks.forEach(link => {
        const sheen = link.querySelector('.fx-sheen');
        link.classList.remove('sweep');
        sheen.classList.remove('sweep');
      });
    }, 600);

    // 安排下一次同步循环
    setTimeout(masterLoop, 4200);
  }

  if (animatedNavLinks.length > 0) {
    setTimeout(masterLoop, 2200); // 延迟启动主循环
  }
});


// =================================================================
//                 SCROLL HINT ARROWS LOGIC
// =================================================================
(function initScrollHint() {
  ...
  let hasClickedNav = false;

  // 检测Hero区是否在视口内
  const heroObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting && !hasScrolled && !hasClickedNav) {
